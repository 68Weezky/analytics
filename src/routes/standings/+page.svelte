...
<svelte:head>
    <title>Hockey K.E - Standings</title>
        ...
        <header class="bg-primary text-white py-4 sticky top-0 z-40">
            <div class="container mx-auto px-4 md:px-8 flex justify-between items-center">
                <div class="flex items-center space-x-4">
                    ...
                    <div class="text-xl font-bold tracking-wide">Hockey K.E Standings</div>
                </div>
                <button
                    type="button"
                    on:click={toggleMenu}
            ...
            <Card.Root>
                <Card.Content>
                    <div class="flex justify-center overflow-x-auto">
                        <table class="w-full max-w-screen-sm md:max-w-full text-sm md:text-base">
                            <thead class="bg-muted">
                                <tr>
                                    <th class="p-2 md:p-4 text-left">Pos</th>
                                    <th class="p-2 md:p-4 text-left">Team</th>
                                    <th class="p-2 md:p-4">MP</th>
                                    <th class="p-2 md:p-4">W</th>
                                    <th class="p-2 md:p-4">D</th>
                                    <th class="p-2 md:p-4">L</th>
                                    <th class="p-2 md:p-4">GF</th>
                                    <th class="p-2 md:p-4">GA</th>
                                    <th class="p-2 md:p-4">GD</th>
                                    <th class="p-2 md:p-4">Pts</th>
                                </tr>
                            </thead>
                            <tbody>
                                {#each standings as team, i}
                                    <tr class={`border-l-4 ${i < 4 ? 'border-blue-500' : i < 6 ? 'border-green-500' : i > 16 ? 'border-red-500' : 'border-transparent'} hover:bg-muted/50`}>
                                        <td class="p-2 md:p-4">{team.position}</td>
                                        <td class="p-2 md:p-4 font-medium">{team.team}</td>
                                        <td class="p-2 md:p-4">{team.played}</td>
                                        <td class="p-2 md:p-4">{team.won}</td>
                                        <td class="p-2 md:p-4">{team.drawn}</td>
                                        <td class="p-2 md:p-4">{team.lost}</td>
                                        <td class="p-2 md:p-4">{team.goalsFor}</td>
                                        <td class="p-2 md:p-4">{team.goalsAgainst}</td>
                                        <td class="p-2 md:p-4">{team.goalDifference}</td>
                                        <td class="p-2 md:p-4 font-bold">{team.points}</td>
                                    </tr>
                                {/each}
                            </tbody>
                        </table>
