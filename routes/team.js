...
router.get('/', async (req, res) => {
    ...
    const myTeam = teams.find(team => team.team_manager === user.name);

    const myTeamPlayers = players.filter(player => player.tm === user.name);

    // console.log(myTeamPlayers);

    // Generate stats
    const stats = await getStats(standings, players, matchesData);
